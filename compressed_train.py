    ...
    with torch.no_grad():
        for iter, (data, gt) in enumerate(data_loader):
            for idx in range(30):
                ...
                refine_out = model(torch.cat([pre.float(),ft1.float()],1)*256)
...
def main(args):
    ...
    model = amct_pytorch.create_compressed_retrain_model(model.cpu(),input_data,args.config_definition,record_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_definition', dest='config_definition', 
                        default="./config/compressed_train/compressed.cfg", type=str, help='The simple configure define file.')
