...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, mid_channels: int, out_channels: int, stride: int = 1):
        ...
        self.conv1 = Conv2D(in_channels, mid_channels, kernel_size=3, stride=stride, padding=1, is_seperable=depth_wise, has_relu=True)
        self.conv2 = Conv2D(mid_channels, out_channels, kernel_size=3, stride=1, padding=1, is_seperable=depth_wise, has_relu=False)
...
class Network(nn.Module):
    ...
    def __init__(self, n_feat=32):
        ...
        self.encdec = Conv2D(in_channels=256, out_channels=64, kernel_size=3, padding=1, stride=1, is_seperable=depth_wise, has_relu=True)
        self.dec1 = DecoderStage(in_channels=64, skip_in_channels=128, out_channels=64)
        self.dec2 = DecoderStage(in_channels=64, skip_in_channels=64, out_channels=32)
        self.dec3 = DecoderStage(in_channels=32, skip_in_channels=32, out_channels=16)
        self.dec4 = DecoderStage(in_channels=16, skip_in_channels=16, out_channels=16)
...
#         self.tri=traninv()
#     def forward(self, x):
#         # tr=self.tr(x,77.50141960574743/65535, 16278.976637616463/(65535**2))
#         post=self.post(tr)
#         # tri = self.tri(post, 77.50141960574743/65535, 16278.976637616463/(65535**2))
#         return tri
