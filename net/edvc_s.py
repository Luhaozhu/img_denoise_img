...
def Conv2D(
    ...
    if is_seperable:
        ...
        modules['conv'] = nn.Conv2d(
            in_channels, out_channels, kernel_size, stride, padding,padding_mode='reflect',
            bias=True,
        )
    if has_relu:
...
if __name__ == "__main__":
    ...
    # model = Network()  # 初始化权重
    model = torch.load("/data/aaron/quantization_deploy/img_denoise/model/edvc_pix.pth")
    model.eval().cpu()
    # tensor = (torch.rand(1, 1, 1024, 1024),)
    onnx_path='edvc_pix.onnx'
    tensor = torch.randn(1, 8, 512, 640)
    # dummy_input = [torch.randn(1, 2, 1024, 1280)
    torch_out = torch.onnx.export(model,tensor,onnx_path,input_names=['input'],output_names=['output'],opset_version=11)
    # 分析FLOPs
