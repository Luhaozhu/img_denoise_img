...
def train(model,train_dataloader,device,output_path,config_file,record_file,input_data):
    model.to(device)
    ...
    for e in range(0, Epoch):
        ...
        #     amct_pytorch.save_quant_retrain_model(config_file,model,record_file,model_output_path,input_data,
...
def main(args):
    ...
    output_path_name = f"{time_now}_epoch200_{args.model_name}_fp32_lr5e-5_notrans"
    ...
    # config_file = "./config/train/config_edvc.json"
    config_file = "./config/train/config_notrans.json"
    record_file = os.path.join(output_path,"record.txt")
    input_data = torch.randn([1,2,512,1280])
    
    # amct_pytorch.create_quant_retrain_config(config_file,model,input_data,args.config_definition)
    ...
    model = amct_pytorch.create_quant_retrain_model(config_file,model,record_file,input_data)

    # Step5 retraining quantitative model and inferencing
    best_model = train(model,train_dataloader,device,output_path,config_file,record_file,input_data)
    ...
    best_model.to("cpu")
    amct_pytorch.save_quant_retrain_model(config_file,best_model,record_file,output_path,input_data,
                                          input_names=['input'], output_names=['output'],
                                          dynamic_axes={'input': {0: 'batch_size'}, 'output': {0: 'batch_size'}})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path",type=str,default="./model/16_Epoch5750-Total_Loss0.0084.pth")
